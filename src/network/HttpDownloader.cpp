...
HttpDownloader::DownloadError runGetWolfSsl(const std::string& url, const std::string& username,
  ...
  for (uint8_t hop = 0; hop < MAX_REDIRECTS; ++hop) {
    ...
    http.setTimeout(timeoutMs > 0 ? timeoutMs : HTTP_TIMEOUT_MS);
...
HttpDownloader::DownloadError runGetDefault(const std::string& url, const std::string& username,
  ...
  for (uint8_t hop = 0; hop < MAX_REDIRECTS; ++hop) {
    ...
    config.timeout_ms = timeoutMs > 0 ? timeoutMs : HTTP_TIMEOUT_MS;
...
HttpDownloader::DownloadError runGet(const std::string& url, const std::string& username, const std::string& password,
  ...
  if (transport == HttpDownloader::Transport::WOLFSSL) {
    return runGetWolfSsl(url, username, password, sink, bufferSize, timeoutMs);
  }
#else
  (void)transport;
#endif
  return runGetDefault(url, username, password, sink, bufferSize, timeoutMs);
}
}  // namespace
...
bool HttpDownloader::fetchUrl(const std::string& url, std::string& outContent, const std::string& username,
                              const std::string& password, CancelCallback shouldCancel, uint32_t timeoutMs) {
  outContent.clear();
  Sink sink;
  ...
  return runGet(url, username, password, sink, DEFAULT_DOWNLOAD_BUFFER_SIZE, Transport::ESP_HTTP, timeoutMs) == OK;
}
