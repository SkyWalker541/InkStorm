...
// The back hint label is "« Back"; the arrow glyph is dropped in landscape so
...
void BaseTheme::drawButtonHints(GfxRenderer& renderer, const char* btn1, const char* btn2, const char* btn3,
  ...
  const bool isLandscape = orig_orientation == GfxRenderer::Orientation::LandscapeClockwise ||
  ...
  if (isLandscape) {
    // The boxes above land on the physical front-button edge, which in
    // landscape is the left/right gutter. Stack each label's letters upright,
    // top-to-bottom down that strip instead of the sideways portrait text.
    int maxChars = 1;
    for (const char* l : labels) {
      if (l != nullptr && l[0] != '\0') maxChars = std::max(maxChars, countVerticalHintChars(l));
    }
    renderer.setOrientation(orig_orientation);
    for (int i = 0; i < 4; i++) {
      if (labels[i] != nullptr && labels[i][0] != '\0') {
        drawVerticalHintText(renderer, Rect{buttonPositions[i], pageHeight - buttonY, buttonWidth, buttonHeight},
                             UI_10_FONT_ID, labels[i], maxChars);
      }
    }
    renderer.setOrientation(orig_orientation);
    return;
...
void BaseTheme::drawButtonHintsSymbols(GfxRenderer& renderer, const ButtonHintSymbol btn1, const ButtonHintSymbol btn2,
  ...
  const ButtonHintSymbol symbols[] = {btn1, btn2, btn3, btn4};

  for (int i = 0; i < 4; i++) {
    if (symbols[i] == ButtonHintSymbol::None) continue;
    const int x = buttonPositions[i];
    renderer.fillRect(x, pageHeight - buttonY, buttonWidth, buttonHeight, false);
    renderer.drawRect(x, pageHeight - buttonY, buttonWidth, buttonHeight);
  }
  ...
  for (int i = 0; i < 4; i++) {
    if (symbols[i] == ButtonHintSymbol::None) continue;
    drawHintSymbol(renderer, Rect{buttonPositions[i], pageHeight - buttonY, buttonWidth, buttonHeight}, symbols[i]);
  }
  renderer.setOrientation(orig_orientation);
}

void BaseTheme::drawHintSymbol(GfxRenderer& renderer, const Rect& portraitBox, const ButtonHintSymbol symbol) const {
  if (symbol == ButtonHintSymbol::None) return;
  ...
  Rect box = portraitBox;
  if (landscape) {
    box = landscapeRectFromPortrait(renderer, portraitBox);
  }

  // Draw in a fixed box centered in the hint area. The renderer applies the
  // current orientation's rotation, so each symbol reads upright in both
  // portrait and landscape views. Strokes are drawn with a 3x3 dilation so
  // they are thick and clearly visible on the small e-ink hint buttons.
  constexpr int s = 22;
  constexpr int w = 3;
  constexpr int hw = w / 2;
  const int bx = box.x + (box.width - s) / 2;
  const int by = box.y + (box.height - s) / 2;

  auto thick = [&renderer](int x1, int y1, int x2, int y2) {
    for (int dx = -hw; dx <= hw; dx++) {
      for (int dy = -hw; dy <= hw; dy++) {
        renderer.drawLine(x1 + dx, y1 + dy, x2 + dx, y2 + dy, true);
      }
    }
  };
  ...
  switch (symbol) {
    case ButtonHintSymbol::Back:
      thick(bx + 6, by + m, bx + s - 2, by + m);  // shaft
      thick(bx + 6, by + 3, bx + 2, by + m);      // top barb
      thick(bx + 2, by + m, bx + 6, by + s - 3);  // bottom barb
      break;
    case ButtonHintSymbol::Menu:
      thick(bx + 2, by + 4, bx + s - 2, by + 4);
      thick(bx + 2, by + m, bx + s - 2, by + m);
      thick(bx + 2, by + s - 4, bx + s - 2, by + s - 4);
      break;
    case ButtonHintSymbol::Up:
      thick(bx + m, by + 3, bx + m, by + s - 3);  // shaft
      thick(bx + 4, by + 8, bx + m, by + 3);      // top barb
    ...
    case ButtonHintSymbol::Ok:
      thick(bx + 3, by + 10, bx + 8, by + s - 3);
      thick(bx + 8, by + s - 3, bx + s - 3, by + 4);
      break;
    case ButtonHintSymbol::Close:
      thick(bx + 3, by + 3, bx + s - 3, by + s - 3);
      thick(bx + s - 3, by + 3, bx + 3, by + s - 3);
      break;
    case ButtonHintSymbol::Refresh: {
      // Bold 3/4 ring (open at the top-right) with a filled arrowhead across
      // the opening. The three quarter-arcs cover 9→12, 9→6 and 6→3 o'clock;
      // the 12→3 gap holds a solid triangle whose tip points clockwise (down),
      // so the glyph reads as a refresh arrow at a glance.
      constexpr int r = 10;
      const int cx = bx + m;
      const int cy = by + m;
      renderer.drawArc(r, cx, cy, -1, -1, 4, true);
      renderer.drawArc(r, cx, cy, -1, +1, 4, true);
      renderer.drawArc(r, cx, cy, +1, +1, 4, true);
      const int ax[] = {cx + 1, cx + 9, cx + 9};
      const int ay[] = {cy - 9, cy - 1, cy + 3};
      renderer.fillPolygon(ax, ay, 3, true);
      break;
    }
    case ButtonHintSymbol::None:
    default:
      break;
  }
}

Rect BaseTheme::landscapeRectFromPortrait(const GfxRenderer& renderer, const Rect& portrait) const {
  const GfxRenderer::Orientation orientation = renderer.getOrientation();
  ...
  if (orientation == GfxRenderer::Orientation::LandscapeCounterClockwise) {
    // Portrait (x,y) → panel (phyX=y, phyY=panelH-1-x); the LCCW view is the
    // panel itself, so the strip is on the right edge.
    return Rect{portrait.y, panelH - portrait.x - portrait.width, portrait.height, portrait.width};
  }
  // LandscapeClockwise: panel is the portrait view rotated 180°, so the strip
  // appears on the left edge.
  return Rect{panelW - portrait.y - portrait.height, portrait.x, portrait.height, portrait.width};
}

int BaseTheme::countVerticalHintChars(const char* label) {
  const std::string text = stripLeadingBackArrow(label);
  if (text.empty()) return 0;
  const unsigned char* p = reinterpret_cast<const unsigned char*>(text.c_str());
  int count = 0;
  while (*p != '\0') {
    if (!utf8IsCombiningMark(utf8NextCodepoint(&p))) count++;
  }
  return count;
}

void BaseTheme::drawVerticalHintText(GfxRenderer& renderer, const Rect& portraitBox, int fontId, const char* label,
                                     const int maxChars) const {
  if (label == nullptr || label[0] == '\0') return;
  const std::string text = stripLeadingBackArrow(label);
  if (text.empty()) return;

  const Rect r = landscapeRectFromPortrait(renderer, portraitBox);
  const int lineHeight = renderer.getLineHeight(fontId);

  // Collect the label's codepoints (skipping combining marks) so the vertical
  // column can be sized and centered before drawing.
  ...
  // Uniform letter spacing shared by every column in the frame, sized from the
  // longest label (maxChars) so the same spacing is used everywhere. Prefer the
  // font's line height so stacked letters never overlap; if even the longest
  // label would then overflow the box, fall back to a fitting spacing (letters
  // tighten, but the column still stays inside its box).
  const int count = static_cast<int>(codepoints.size());
  const int n = std::max(maxChars, count);
  const int fitSpacing = std::max(1, r.height / (n + 1));
  int spacing = std::max(lineHeight, fitSpacing);
  if ((n - 1) * spacing + lineHeight > r.height) {
    spacing = fitSpacing;
  }
  const int columnHeight = (count - 1) * spacing + lineHeight;
  int baseline = r.y + (r.height - columnHeight) / 2 + lineHeight;

  std::string single;
  for (const uint32_t cp : codepoints) {
    utf8AppendCodepoint(cp, single);
    const int charW = renderer.getTextWidth(fontId, single.c_str());
    renderer.drawText(fontId, r.x + (r.width - charW) / 2, baseline, single.c_str());
    single.clear();
    baseline += spacing;
  }
}
...
void BaseTheme::drawHeader(const GfxRenderer& renderer, Rect rect, const char* title, const char* subtitle,
                           const bool readerContext, const bool drawClock) const {
  ...
  const bool showHeaderClock =
      drawClock && halClock.isAvailable() &&
  ...
  // When the caller draws the header without the theme clock (drawClock==false,
  // e.g. the weather app draws its own time), it still expects the left-aligned
  // title layout a visible clock would normally produce.
  const bool titleLeft = hasVisibleTitle && (drawClock ? showHeaderClock : true);
  props.titleText.align = titleLeft ? fui::TextAlign::Left : tokens.headerTitleAlign;
