...
#include "components/ButtonHintSymbol.h"
...
class BaseTheme {
 public:
  ...
  // Symbol variant of drawButtonHints: each non-None symbol is drawn centered
  // in its hint box. The boxes use the same geometry as drawButtonHints; in
  // landscape each symbol is centered in the mapped gutter strip, oriented so
  // it reads upright in the landscape view.
  virtual void drawButtonHintsSymbols(GfxRenderer& renderer, ButtonHintSymbol btn1, ButtonHintSymbol btn2,
                                      ButtonHintSymbol btn3, ButtonHintSymbol btn4) const;
  ...
  virtual void drawHeader(const GfxRenderer& renderer, Rect rect, const char* title, const char* subtitle = nullptr,
                          bool readerContext = false, bool drawClock = true) const;
  ...
  // Landscape hint-bar helpers. The front-button hint boxes are always drawn
  // through the portrait pass (they land on the physical edge that appears as
  // the left/right gutter in landscape); these map a portrait button rect into
  // the current landscape view and draw its label as upright letters stacked
  // top-to-bottom down the strip (like "M/e/n/u"), centered in the box.
  // maxChars is the length (in codepoints) of the longest of the four labels
  // for this frame, so every column uses the same spacing and fits its box.
  Rect landscapeRectFromPortrait(const GfxRenderer& renderer, const Rect& portrait) const;
  void drawVerticalHintText(GfxRenderer& renderer, const Rect& portraitBox, int fontId, const char* label,
                            int maxChars) const;
  // Draws one hint symbol centered in its box. In portrait `portraitBox` is
  // the box itself; in landscape it is mapped via landscapeRectFromPortrait
  // and the symbol is centered in the gutter strip. The caller must have the
  // renderer in the frame's orientation (Portrait or landscape).
  void drawHintSymbol(GfxRenderer& renderer, const Rect& portraitBox, ButtonHintSymbol symbol) const;
  // Codepoint count a label contributes to a vertical hint column, ignoring a
  // leading "«" (the back arrow, which is dropped in landscape).
  static int countVerticalHintChars(const char* label);
};
