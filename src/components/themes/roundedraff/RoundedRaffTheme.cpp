...
void RoundedRaffTheme::drawHeader(const GfxRenderer& renderer, Rect rect, const char* title, const char* subtitle,
  ...
  BaseTheme::drawHeader(renderer, rect, title, subtitle, readerContext, drawClock);
}
...
void RoundedRaffTheme::drawButtonHints(GfxRenderer& renderer, const char* btn1, const char* btn2, const char* btn3,
  ...
  const bool isLandscape = origOrientation == GfxRenderer::Orientation::LandscapeClockwise ||
                           origOrientation == GfxRenderer::Orientation::LandscapeCounterClockwise;
  ...
  const int lineHeight = renderer.getLineHeight(kGuideFontId);
  const int textBaseline = (invertText ? bottomMargin : outlineY) + (hintHeight - lineHeight) / 2;

  if (isLandscape) {
    // The groups above land on the physical front-button edge, which in
    ...
    int maxChars = 1;
    for (const std::string& l : {backLabel, selectText, upText, downText}) {
      if (!l.empty()) maxChars = std::max(maxChars, countVerticalHintChars(l.c_str()));
    }
    // Each group strip holds two labels; give each column a third of the strip
    // so the letters center in a comfortably sized area (and never overflow).
    const int columnArea = groupWidth / 3;
    renderer.setOrientation(origOrientation);
    const Rect backBox{backX, textBaseline - lineHeight, columnArea, lineHeight};
    const Rect selectBox{selectX, textBaseline - lineHeight, columnArea, lineHeight};
    const Rect upBox{upX, textBaseline - lineHeight, columnArea, lineHeight};
    const Rect downBox{downX, textBaseline - lineHeight, columnArea, lineHeight};
    if (!backDisabled) drawVerticalHintText(renderer, backBox, kGuideFontId, backLabel.c_str(), maxChars);
    drawVerticalHintText(renderer, selectBox, kGuideFontId, selectText.c_str(), maxChars);
    drawVerticalHintText(renderer, upBox, kGuideFontId, upText.c_str(), maxChars);
    drawVerticalHintText(renderer, downBox, kGuideFontId, downText.c_str(), maxChars);
    renderer.setOrientation(origOrientation);
    return;
  }

  renderer.setOrientation(invertText ? GfxRenderer::Orientation::PortraitInverted : GfxRenderer::Orientation::Portrait);
  const int textY = textBaseline;
...
void RoundedRaffTheme::drawButtonHintsSymbols(GfxRenderer& renderer, const ButtonHintSymbol btn1,
  ...
  // Back and Select occupy the left group, Up and Down the right group.
  const ButtonHintSymbol leftSymbols[] = {btn1, btn2};
  const ButtonHintSymbol rightSymbols[] = {btn3, btn4};
  ...
  renderer.setOrientation(origOrientation);
  const Rect leftGroup{leftGroupX, outlineY, groupWidth, hintHeight};
  const Rect rightGroup{rightGroupX, outlineY, groupWidth, hintHeight};
  for (int i = 0; i < 2; i++) {
    if (leftSymbols[i] != ButtonHintSymbol::None) drawHintSymbol(renderer, leftGroup, leftSymbols[i]);
    if (rightSymbols[i] != ButtonHintSymbol::None) drawHintSymbol(renderer, rightGroup, rightSymbols[i]);
  }
  renderer.setOrientation(origOrientation);
}
