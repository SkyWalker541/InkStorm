...
class OptionPopup {
 public:
  // Allow the first button hint to be customized (default is Back).
  void setBackSymbol(ButtonHintSymbol symbol) { backSymbol = symbol; }
  ...
  bool handleInput(MappedInputManager& input, const std::function<void()>& requestUpdate) {
    ...
    if (input.wasPressed(MappedInputManager::Button::Up) || input.wasPressed(MappedInputManager::Button::Left)) {
      ...
      // Ignore a Confirm/Back pressed immediately after the popup opened: the
      // popup opens on the release of the row's Confirm, so a quick second tap
      // would instantly close it again with the value unchanged (and the user
      // often expects that tap to be a no-op while they decide).
      if ((millis() - activatedAtMs) < kIgnoreImmediatePressMs) return true;
      active = false;
  ...
  // Same as processRender but draws symbol hint buttons (used by the weather
 ...
 private:
  ...
  // Window after a popup opens during which a Confirm/Back press is ignored
  // (see handleInput). Fixes accidental instant-close on a quick second tap.
  ...
  ButtonHintSymbol backSymbol = ButtonHintSymbol::Back;
