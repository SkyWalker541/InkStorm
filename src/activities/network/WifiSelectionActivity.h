...
class WifiSelectionActivity final : public Activity {
  ...
  // When set, render() forces this orientation for the whole frame. Used by the
  // weather flow so the WiFi/connecting screen matches the weather orientation.
  const std::optional<GfxRenderer::Orientation> forcedOrientation;

  // Weather-launched WiFi popups use symbol hints (X / checkmark / arrows)
  // instead of text labels so they match the weather UI style.
  const bool useWeatherButtonHints;
 ...
 public:
  explicit WifiSelectionActivity(GfxRenderer& renderer, MappedInputManager& mappedInput, bool autoConnect = true,
                                 ...
                                 std::optional<GfxRenderer::Orientation> forcedOrientation = std::nullopt,
                                 bool useWeatherButtonHints = false);
  void onEnter() override;
  void onExit() override;
  void loop() override;
  void render(RenderLock&&) override;

  // A scan or connection attempt is background work with no user interaction;
  // keep the device awake so a parent flow (e.g. weather auto-refresh) parked
  // behind this screen does not hit the inactivity sleep while it is waiting.
  bool preventAutoSleep() override {
    return state == WifiSelectionState::SCANNING || state == WifiSelectionState::AUTO_CONNECTING ||
