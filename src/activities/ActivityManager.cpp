...
void ActivityManager::renderTaskLoop() {
  while (true) {
    ...
    if (currentActivity) {
      // Never render into a null framebuffer: an activity that lent its buffer
      // to the heap (e.g. weather fetch) and failed to reallocate it would
      // otherwise fault on the first drawPixel. Skip the frame instead; the
      // lending activity recovers the buffer before re-requesting a render.
      if (renderer.getFrameBuffer() == nullptr) {
        LOG_ERR("ACT", "Skipping render: framebuffer is null (realloc pending)");
      } else {
...
#ifdef INKSTORM_LEAN
  // The weather build has no Settings menu behind the power button; weather
  // owns the power button entirely.
  return false;
#else
...
#ifdef INKSTORM_LEAN
  // The lean weather build has no bookshelf. Popping the last activity returns
  // to the weather home screen — never to a hidden CrossInk home.
