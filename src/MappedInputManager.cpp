...
MappedInputManager::Symbols MappedInputManager::mapSymbols(const ButtonHintSymbol back, const ButtonHintSymbol confirm,
  ...
  auto symbolForHardware = [&](ButtonIndex hw) -> ButtonHintSymbol {
    if (hw == mappedBack) return back;
    if (hw == mappedConfirm) return confirm;
    if (hw == mappedLeft) return previous;
    if (hw == mappedRight) return next;
    return ButtonHintSymbol::None;
  };

  return {symbolForHardware(HalGPIO::BTN_BACK), symbolForHardware(HalGPIO::BTN_CONFIRM),
          symbolForHardware(HalGPIO::BTN_LEFT), symbolForHardware(HalGPIO::BTN_RIGHT)};
}
