...
CrossPointSettings::SHORT_PWRBTN getPowerButtonAction() {
  ...
  // On the InkStorm (weather) partition the power button is handled entirely
  // by the weather activity: a single tap refreshes the forecast and a long
  // press is ignored (sleep is disabled). Never consume the button edge here.
...
void enterDeepSleep(bool fromTimeout) {
  // Sleep is 100% disabled on the InkStorm (weather) partition. The power
  // button acts as a manual weather refresh instead, so never enter deep
  // sleep here regardless of which path requested it.
  if (isRunningInkStorm()) {
    LOG_INF("MAIN", "Sleep disabled on InkStorm partition; skipping deep sleep");
    return;
  }
#ifndef INKSTORM_LEAN
  ...
  // If InkStorm is sleeping, ensure the next wake boots back into CrossInk.
  if (isRunningInkStorm()) {
    const esp_partition_t* target =
        esp_partition_find_first(ESP_PARTITION_TYPE_APP, ESP_PARTITION_SUBTYPE_APP_OTA_0, nullptr);
    if (target) {
      esp_ota_set_boot_partition(target);
      LOG_INF("MAIN", "InkStorm sleep: switched boot partition to CrossInk (app0)");
    }
  }
...
void registerOtaAppName(const char* name) {
  ...
  nvs_handle_t handle;
  if (nvs_open("ota_names", NVS_READWRITE, &handle) != ESP_OK) return;
  nvs_set_str(handle, key, name);
  nvs_commit(handle);
  nvs_close(handle);
  LOG_INF("BOOT", "Registered OTA app name \"%s\" in slot %d", name, slot);
}
#endif
...
void setup() {
...
#ifndef SIMULATOR
  registerOtaAppName("CrossInk v1.5.0 / InkStorm v1.0.0 Dual Boot");
#endif
  ...
  const bool isNetworkResume = snapshotTarget >= static_cast<uint32_t>(NetworkBootTarget::OPDS);
  silentRebootMagic = 0;
  ...
  // Seed the ESP32 system clock from the DS3231 so time() (used by weather
  // day-of-week/"updated", file timestamps, block-due tracking) agrees with the
  // device clock even before the one-time NTP sync. Guard against a stale RTC.
  {
    time_t rtcEpoch = 0;
    if (halClock.getEpoch(rtcEpoch) && rtcEpoch > 1609459200) {  // > 2021-01-01
      struct timeval tv = {static_cast<time_t>(rtcEpoch), 0};
      settimeofday(&tv, nullptr);
      LOG_INF("CLK", "Seeded system time from RTC: %lld", (long long)rtcEpoch);
    }
  }
  ...
  switch (wakeupReason) {
    case HalGPIO::WakeupReason::PowerButton: {
      ...
      LOG_INF("BOOT", "Power-button wake: verifying duration required=%u shortAllowed=%d", requiredDuration,
              shortPressWakes);
      if (!gpio.verifyPowerButtonWakeup(requiredDuration, shortPressWakes) && !isRunningInkStorm()) {
        // On the InkStorm partition sleep is disabled entirely, so never send
        // the device back to deep sleep — proceed to boot instead.
        powerManager.startDeepSleep(gpio);
      }
      break;
    }
  ...
  if (HalSystem::isRebootFromPanic()) {
    ...
    // The lean weather binary contains no reader or home, so every boot — on
    // either partition — launches the weather app.
    LOG_INF("BOOT", "InkStorm (lean) build — launching WeatherActivity");
    ...
    switch (static_cast<NetworkBootTarget>(snapshotTarget)) {
      // OTA update removed in dual-boot build.
  ...
  } else if (isRunningInkStorm()) {
    // InkStorm partition (app1): boot straight into weather app.
    LOG_INF("BOOT", "InkStorm partition detected — launching WeatherActivity");
    activityManager.replaceActivity(std::make_unique<WeatherActivity>(renderer, mappedInputManager));
