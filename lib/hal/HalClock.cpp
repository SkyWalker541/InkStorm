...
bool HalClock::getEpoch(time_t& epoch) const {
  ...
  // Days-from-civil algorithm (Howard Hinnant), computed in UTC.
  int y = year;
  int m = month;
  int d = day;
  y -= m <= 2;
  ...
  const time_t days = static_cast<time_t>(era) * 146097 + static_cast<time_t>(doe) - 719468;
  epoch = days * 86400 + static_cast<time_t>(hour) * 3600 + static_cast<time_t>(minute) * 60;
  return true;
}
