...
class HalClock {
 ...
 public:
  ...
  // Returns the RTC's raw UTC time as a Unix epoch (seconds). Returns false if
  // the RTC is unavailable or its date is invalid. Used to seed the ESP32
  // system clock at boot so time() agrees with the displayed device clock.
