...
class GfxRenderer {
 ...
 public:
  ...
  /// Distance below the baseline of the lowest glyph (positive magnitude).
  ...
  // Lend the display's framebuffer storage back to the heap during a
  // memory-hungry phase (e.g. a TLS fetch). No render/display calls may run
  // between releaseFramebuffers() and a successful reallocFramebuffers().
...
// Lends the single ~52 KB framebuffer back to the heap for the lifetime of a
// memory-hungry, display-free phase (e.g. EPUB chapter layout). No render or
// display calls may run while released; drawing to the screen resumes only
// after a successful restore(). Constructing it is a no-op when no framebuffer
...
class FramebufferReleaseScope {
 public:
  ...
  // Reallocate the framebuffer back now (rather than waiting for the
  // destructor) and report success; a failed restore means the host is
  // critically low and must not attempt any further rendering.
  bool restore() {
    if (!active_) return true;
    if (renderer_.reallocFramebuffers()) {
      active_ = false;
      return true;
    }
    return false;
  ...
  bool released() const { return active_; }

  ~FramebufferReleaseScope() {
    if (active_) {
      renderer_.reallocFramebuffers();
    }
  }
