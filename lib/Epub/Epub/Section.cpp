...
bool Section::createSectionFile(const ReaderRenderSpec& spec, const std::function<void()>& popupFn,
  ...
  // The single ~52 KB framebuffer is idle while this chapter is laid out:
  // pages serialize to the SD cache and only the (optional) indexing popupFn
  // draws. Hand it back to the heap for the duration so large chapters keep
  // the contiguous memory the parser's low-memory abort gates require
  // (44 KB free / 32 KB max-alloc). popupFn is suppressed while released and
  // the caller's pre-build popup simply stays frozen on the panel; rendering
  // resumes once restore() reallocates the buffer.
  FramebufferReleaseScope fbScope(renderer);
  const bool fbReleased = fbScope.released();
  ...
  ChapterHtmlSlimParser visitor(
      *epub, parsePath, renderer, fontId, lineCompression, extraParagraphSpacing, forceParagraphIndents,
      ...
      fbReleased ? std::function<void()>{} : popupFn, cssParser, renderMode,
  ...
  // The caller (and any popup redraw) needs the framebuffer back. The build
  // just ran with it freed, so its transient allocations are gone; give a
  // short grace period for the freed block to coalesce, then fail the build
  // as a low-memory abort if the device is too fragmented to reallocate.
  if (fbReleased) {
    const uint32_t reallocDeadline = millis() + 2000;
    bool fbRestored = fbScope.restore();
    while (!fbRestored && millis() < reallocDeadline) {
      delay(25);
      fbRestored = fbScope.restore();
    }
    if (!fbRestored) {
      LOG_ERR("SCT", "Framebuffer realloc failed after section build (free=%u maxAlloc=%u)", ESP.getFreeHeap(),
              ESP.getMaxAllocHeap());
      if (layoutAbortedForLowMemory) *layoutAbortedForLowMemory = true;
      file.close();
      Storage.remove(tmpSectionPath.c_str());
